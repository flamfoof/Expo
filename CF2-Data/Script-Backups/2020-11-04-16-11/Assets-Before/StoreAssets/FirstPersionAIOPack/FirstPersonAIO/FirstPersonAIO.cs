...
public class FirstPersonAIO : MonoBehaviour {
    ...
    private void Awake(){
        #region Input Init - Awake
        //inputAction = new PlayerInp();
        #endregion
...
*/
    ...
    private void FixedUpdate(){
        ...
        XYInput = new Vector2(Input.GetAxis("Mouse X") , Input.GetAxis("Mouse Y"));
        #endregion
        ...
        /*
        if(useStamina){
            isSprinting = sprintKey && !isCrouching && staminaInternal > 0 && (Mathf.Abs(fps_Rigidbody.velocity.x) > 0.01f || Mathf.Abs(fps_Rigidbody.velocity.z) > 0.01f);
            if(isSprinting){
            ...
            }else if((!sprintKey||Mathf.Abs(fps_Rigidbody.velocity.x)< 0.01f || Mathf.Abs(fps_Rigidbody.velocity.z)< 0.01f || isCrouching)&&staminaInternal<staminaLevel){
